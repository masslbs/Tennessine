...
import { hardhat } from "jsr:@wevm/viem/chains";
...
let relayEndpoint;
let shopId = random32BytesHex();
const windowLocation = typeof window == "undefined"
  ? undefined
  : new URL(window.location.href);

const relayURL = (Deno && Deno.env.get("RELAY_ENDPOINT")) ||
...
describe({
  ...
  fn() {
    ...
    let relayClient;
    test("should create relay client", async () => {
...
describe({
  ...
  fn() {
    ...
    test("should connect and disconnect", async () => {
      const authenticated = await relayClient.connect(true);
    ...
    describe("invite another user", () => {
      ...
      beforeEach(async () => {
        ...
        relayClient2 = await createRelayClient(sk);
        ...
        await relayClient2.connect(true);
...
describe({
  name:
    "If there is a network error, state manager should not change the state.",
  sanitizeResources: false,
  sanitizeOps: false,
  fn() {
