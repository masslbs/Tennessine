...
export const buildState = (
  ...
  events.map((e) => {
    if (e.updateManifest) {
      ...
      if (e.updateItem.field == ItemField.ITEM_FIELD_METADATA && e.updateItem.metadata) {
      ...
      } else if (e.updateItem.field == ItemField.ITEM_FIELD_PRICE && e.updateItem.price) {
        productsDispatch({
          type: UPDATE_PRICE,
          payload: {
            itemId: id,
      ...
      evt.itemIds?.length && evt.itemIds.map((id, i) => {
