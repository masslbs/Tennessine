...
const relayEndpoint = (process && process.env["RELAY_ENDPOINT"]) || "ws://localhost:4444/v1";
let relayClient;
const storeId = `0x${randomBytes(32).toString("hex")}`;
...
describe("RelayClient", async () => {
    ...
    test("regstrationTokenRedeem", async () => {
        ...
        const hash = await relayClient.blockchain.redeemInviteSecret(sk, client2Wallet);
...
describe("user behaviour", () => {
    ...
    test("update store manifest", async () => {
        await relayClient.updateManifest(ManifestField.MANIFEST_FIELD_DOMAIN, "test");
        await relayClient.updateManifest(ManifestField.MANIFEST_FIELD_DOMAIN, "socks.mass.market");
        await relayClient.updateManifest(ManifestField.MANIFEST_FIELD_ADD_ERC20, abi.addresses.Eddies);
        await relayClient.updateManifest(ManifestField.MANIFEST_FIELD_REMOVE_ERC20, abi.addresses.Eddies);
    });
    ...
    describe("editing the listing", () => {
        ...
        test("update item - price", async () => {
            await relayClient.updateItem(itemId, mmproto.UpdateItem.ItemField.ITEM_FIELD_PRICE, "20.99");
            expect(itemId).not.toBeNull();
        });
        describe("tagging", () => {
            let tagId;
        ...
        describe("checkout process", () => {
            ...
            test("erc20 checkout", { timeout: 10000 }, async () => {
                await relayClient.updateManifest(ManifestField.MANIFEST_FIELD_ADD_ERC20, abi.addresses.Eddies);
                await relayClient.changeCart(cartId, itemId, 1);
                const checkout = await relayClient.commitCart(cartId, abi.addresses.Eddies);
                expect(checkout).not.toBeNull();
                expect(checkout.cartFinalizedId).not.toBeNull();
            });
        });
    ...
    describe("invite another user", { retry: 3 }, async () => {
        ...
        test("client2 successfully updates manifest", async () => {
            await relayClient2.updateManifest(ManifestField.MANIFEST_FIELD_DOMAIN, "test2-test");
